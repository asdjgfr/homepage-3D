...
class MultiPrefabBufferGeometry extends BufferGeometry {
  ...
  constructor(prefabs, repeatCount) {
     ...
     * @type {Array}
     */
    this.prefabVertexCounts = this.prefabGeometries.map((p) =>
      p.isBufferGeometry ? p.attributes.position.count : p.vertices.length
    );
    /**
  ...
  setPrefabData(attribute, prefabIndex, data) {
    ...
    const prefabGeometryVertexCount =
      this.prefabVertexCounts[prefabGeometryIndex];
    const whole =
...
const TimelineChunks = {
  ...
  progress: function (segment) {
    ...
    if (segment.duration === 0) {
      ...
      return `
      float progress = clamp(time - cDelay${segment.key}, 0.0, cDuration${
        segment.key
      }) / cDuration${segment.key};
      ${
        segment.transition.ease
          ? `progress = ${segment.transition.ease}(progress${
...
const TranslationSegment = {
  compiler: function (segment) {
    ...
    void applyTransform${segment.key}(float time, inout vec3 v) {
      ...
      v += mix(cTranslateFrom${segment.key}, cTranslateTo${
      segment.key
    }, progress);
    }
    `;
...
const RotationSegment = {
  compiler(segment) {
    ...
    void applyTransform${segment.key}(float time, inout vec3 v) {
      ...
      ${origin ? `v -= cOrigin${segment.key};` : ""}
      vec3 axis = normalize(mix(cRotationFrom${segment.key}.xyz, cRotationTo${
      segment.key
    }.xyz, progress));
      float angle = mix(cRotationFrom${segment.key}.w, cRotationTo${
      segment.key
    }.w, progress);
